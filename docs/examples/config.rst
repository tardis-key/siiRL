...
The configuration parameters detailed in this section are consistent with the `verl` (Volcano Engine Reinforcement Learning for LLMs) framework, as we adhere to the same design principles and specifications. [1]_
...
``lr_decay_steps``. However, in siiRL, to align with FSDP's default behavior, we set the default
...
The `workflow_grpo.yaml` file defines the entire computational graph for the GRPO algorithm as a Directed Acyclic Graph (DAG). This powerful abstraction allows for the rapid implementation of custom RL workflows by specifying each step and its dependencies. The framework's DAG Planner then serializes this graph to ensure efficient execution on a colocated resource pool.
...
- ``dag_id``: A unique identifier for the entire workflow.
- ``description``: A human-readable summary of the workflow's purpose.
- ``nodes``: A list defining the computational steps that form the DAG. Each node has several key attributes:
  - ``node_id``: A unique, user-defined name for the node (e.g., `rollout_actor`).
  - ``node_type``: Specifies the nature of the computation, as defined in the ``NodeType`` enum in `node.py`. Common types include:

    - ``MODEL_INFERENCE``: A forward pass using a model, typically for generating data (rollouts).
    - ``MODEL_TRAIN``: A training step that involves model weights, potentially including forward and backward passes.
    - ``COMPUTE``: A CPU-bound task for numerical calculations (e.g., GAE, function-based rewards).
    - ``DATA_LOAD``: Loads a batch of data from the data loader.
    - ``ENV_INTERACT``: Interacts with an external environment (for standard RL tasks).
    - ``PUT_TO_BUFFER`` / ``GET_FROM_BUFFER``: Operations for interacting with a distributed data buffer.
    - ``BARRIER_SYNC``: A synchronization point for all workers.

  - ``node_role``: Defines the functional role of the worker executing the task, as defined in the ``NodeRole`` enum in `node.py`. This helps with resource allocation and logic mapping. Supported roles include:

    - ``ACTOR``: The policy model being trained.
    - ``CRITIC``: The value function model.
    - ``REFERENCE``: The frozen reference policy model used for KL-divergence calculation.
    - ``ROLLOUT``: The worker responsible for generating experience by running the policy.
    - ``REWARD``: The worker that computes rewards.
    - ``ADVANTAGE``: The worker that calculates advantages (e.g., GAE).
    - ``DEFAULT``: A generic role for nodes without a specific RL function.

  - ``dependencies``: A list of `node_id`s that must be completed before this node can execute. This defines the structure and execution order of the graph.
  - ``only_forward_compute``: An optional flag for `MODEL_TRAIN` nodes, indicating that only a forward pass should be performed without any parameter updates. This is useful for steps like calculating log probabilities with an old policy.

.. [1] https://verl.readthedocs.io/en/latest/examples/config.html