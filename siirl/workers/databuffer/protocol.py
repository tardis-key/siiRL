...
__all__ = ["DataProto", "union_tensor_dict", "pad_dataproto_to_divisor", "unpad_dataproto"]
...
class DataProto:
    ...
    #     This method supports automatic padding for tensors with variable-length dimensions
    #     (e.g., sequence length). It determines padding values and padding side from
    #     `data[0].meta_info['padding_values']` and `data[0].meta_info['padding_side']`.
    ...
    #     # --- Handle non-tensor data first ---
    #     non_tensor_batch = list_of_dict_to_dict_of_list(list_of_dict=[d.non_tensor_batch for d in data])
    #     for key, val in non_tensor_batch.items():
    #         if len(val) > 0 and all(isinstance(v, np.ndarray) for v in val):
    #             non_tensor_batch[key] = np.concatenate(val, axis=0)

    #     # --- Handle tensor data with automatic padding ---
    #     batch_lst = [d.batch for d in data if d.batch is not None and len(d.batch.keys()) > 0]
    #     new_batch = None

    #     if batch_lst:
    #         # 1. Find all keys and determine max shapes for each dimension > 0
    #         all_keys = set()
    #         for td in batch_lst:
    ...
    #         max_shapes = {}
    #         for key in all_keys:
    #             shapes = [td[key].shape for td in batch_lst if key in td.keys()]
    #             if not shapes or len(shapes[0]) <= 1:
    #                 continue

    #             rank = len(shapes[0])
    #             max_shape_dims = [max(s[i] for s in shapes) for i in range(1, rank)]
    #             max_shapes[key] = max_shape_dims

    #         # 2. Get padding configuration from meta_info
    #         padding_map = data[0].meta_info.get("padding_values", {})
    #         padding_side = data[0].meta_info.get("padding_side", "right")

    #         # 3. Pad each tensor and collect
    #         padded_tensors_by_key = {key: [] for key in all_keys}
    #         valid_keys = set(all_keys)

    #         for td in batch_lst:
    #             for key in list(valid_keys):
    #                 if key not in td:
    #                     # If a key is missing, it's an inconsistent schema.
    #                     # We'll stop trying to process this key for all DataProtos.
    #                     valid_keys.remove(key)
    #                     del padded_tensors_by_key[key]

    #         for td in batch_lst:
    #             for key in valid_keys:
    #                 tensor = td[key]
    #                 if key in max_shapes:
    #                     target_shape_dims = max_shapes[key]
    #                     padding = []
    #                     # Iterate dims from last to first (non-batch part of shape)
    #                     for i in range(tensor.dim() - 1, 0, -1):
    #                         pad_needed = target_shape_dims[i-1] - tensor.shape[i]
    #                         if padding_side == "left":
    #                             padding.extend([pad_needed, 0])
    #                         else: # right padding
    #                             padding.extend([0, pad_needed])

    #                     if any(p > 0 for p in padding):
    #                         padding_value = padding_map.get(key, 0.0)
    #                         padded_tensor = F.pad(tensor, padding, "constant", padding_value)
    #                     else:
    #                         padded_tensor = tensor
    #                 else:
    #                     padded_tensor = tensor

    #                 padded_tensors_by_key[key].append(padded_tensor)

    #         # 4. Concatenate the lists of padded tensors
    #         final_tensors = {}
    #         for key, tensor_list in padded_tensors_by_key.items():
    #             if tensor_list:
    #                 final_tensors[key] = torch.cat(tensor_list, dim=0)

    #         # 5. Create the new TensorDict
    #         batch_size = sum(td.batch_size[0] for td in batch_lst)
    #         new_batch = TensorDict(source=final_tensors, batch_size=[batch_size])

    #     meta_info = data[0].meta_info
    #     cls = type(data[0])
    #     return cls(batch=new_batch, non_tensor_batch=non_tensor_batch, meta_info=meta_info)

    def subset(self, indices: Union[torch.Tensor, np.ndarray, List[int]]) -> "DataProto":
        """Creates a new DataProto instance containing a subset of the data.

        This method correctly preserves the type of the `batch` attribute
        (e.g., TensorDict) by directly slicing it, which maintains all its
        special attributes like `.batch_size`.

        Args:
            indices: A tensor, array, or list of integers indicating which
                     items to include in the subset.

        Returns:
            A new DataProto instance containing only the selected data, with
            the correct batch type preserved.
        """
        # Directly slice the TensorDict-like object. This is the idiomatic
        # way and preserves the object's class and attributes.
        new_batch = self.batch[indices]

        # The logic for non_tensor_batch can remain the same, as it's
        # likely a standard dictionary of numpy arrays or lists.
        new_non_tensor_batch = {}
        for key, arr in self.non_tensor_batch.items():
            if isinstance(arr, np.ndarray):
                new_non_tensor_batch[key] = arr[indices]
            else:
                # Handle lists
                new_non_tensor_batch[key] = [arr[i] for i in indices]

        # Deepcopy meta_info to prevent side effects on the original object.
        # The constructor call should match your DataProto's definition.
        return DataProto(batch=new_batch, non_tensor_batch=new_non_tensor_batch, meta_info=deepcopy(self.meta_info))

    @staticmethod
    def concat(data: List["DataProto"]) -> "DataProto":
        """
        Concatenates a list of DataProto objects into a single one. This optimized
        version uses a single pass to gather tensor shapes and data, significantly
        improving performance by reducing Python loop overhead.
        ...
        Args:
            data (List[DataProto]): A list of DataProto objects to concatenate.

        Returns:
            DataProto: A single, concatenated DataProto object.
        """
        if not data:
            return DataProto()

        # --- [OPTIMIZED] Single Pass Data Collection ---
        # We iterate through the list of DataProtos only once to collect all necessary information.

        all_non_tensor_parts = []
        all_tensor_dicts = []
        all_tensor_keys = set()
        max_shapes = {}
        total_batch_size = 0
        ...
        for dp in data:
            if dp.non_tensor_batch:
                all_non_tensor_parts.append(dp.non_tensor_batch)

            if dp.batch is not None and len(dp.batch.keys()) > 0:
                batch_td = dp.batch
                all_tensor_dicts.append(batch_td)
                total_batch_size += batch_td.batch_size[0]

                # In the same loop, find all keys and update max shapes on the fly.
                current_keys = batch_td.keys()
                all_tensor_keys.update(current_keys)

                for key in current_keys:
                    tensor = batch_td[key]
                    if tensor.dim() > 1:
                        if key not in max_shapes:
                            # Initialize with the shape of the first tensor found for this key.
                            max_shapes[key] = list(tensor.shape[1:])
                        else:
                            # Update max shape dimensions for each non-batch dimension.
                            for i in range(len(max_shapes[key])):
                                max_shapes[key][i] = max(max_shapes[key][i], tensor.shape[i + 1])

        # --- Process Non-Tensor Data ---
        final_non_tensor_batch = {}
        if all_non_tensor_parts:
            final_non_tensor_batch = list_of_dict_to_dict_of_list(list_of_dict=all_non_tensor_parts)
            for key, val_list in final_non_tensor_batch.items():
                if val_list and all(isinstance(v, np.ndarray) for v in val_list):
                    final_non_tensor_batch[key] = np.concatenate(val_list, axis=0)
        ...
        if all_tensor_dicts:
            ...
            padded_tensors_by_key = {key: [] for key in all_tensor_keys}

            # This second pass for padding is necessary after all max_shapes are determined.
            for td in all_tensor_dicts:
                for key in all_tensor_keys:
                    if key not in td:
                        # Assuming consistent schema; if a key is missing, this could be an issue.
                        # For robustness, one could create and pad a tensor of zeros here,
                        # but that requires knowing dtype and device, adding complexity.
                        # We follow the original implicit assumption of consistent keys.
                        continue
                    ...
                    if key in max_shapes:
                        target_shape_dims = max_shapes[key]
                        # Create padding tuple in reverse order for F.pad: (pad_dim_N, ..., pad_dim_1)
                        padding = []
                        for i in range(tensor.dim() - 1, 0, -1):
                            pad_needed = target_shape_dims[i - 1] - tensor.shape[i]
                            if padding_side == "left":
            ...
            final_tensors = {key: torch.cat(t_list, dim=0) for key, t_list in padded_tensors_by_key.items() if t_list}

            if final_tensors:
                final_batch = TensorDict(source=final_tensors, batch_size=[total_batch_size])

        meta_info = data[0].meta_info
        cls = type(data[0])
        return cls(batch=final_batch, non_tensor_batch=final_non_tensor_batch, meta_info=meta_info)
