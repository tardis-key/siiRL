    ...
    def __call__(self, data: DataProto, return_dict: bool = False):
        ...
        for i in range(len(data)):
            ...
            if self.rank == 0 and already_print_data_sources[data_source] < self.num_examine:
                ...
                response_str = self.tokenizer.decode(data[i].batch["responses"], skip_special_tokens=True)
                print("[response]", response_str)
                print("[ground_truth]", data[i].non_tensor_batch["reward_model"]["ground_truth"])
